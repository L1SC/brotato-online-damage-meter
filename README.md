...
其他版本尚未确认兼容。测试记录与未验证项见 [验证说明](docs/online-damage-meter-verification.md)。
...
- **缺少前置或版本不符**：核对前置 Mod ID、启用状态和上述版本，启用后重启游戏。
 - **客户端仍无统计**：确认已进入实际战斗且确有伤害产生；记录客户端/房主身份、游戏与 Mod 版本、人数、波次及游戏日志，按 [验证说明](docs/online-damage-meter-verification.md) 核对。房主不需要安装此补丁，但两个前置仍需按原要求安装。
...
构建与自动测试不能代替实际跨机器联机验证。客户端单独安装、多人显示、波次切换以及武器引用刷新等结果，以 [验证说明](docs/online-damage-meter-verification.md) 为准。
